...
gdrive-backup() {
  ...
  echo "☁️ Backup Workspace → Google Drive"
  echo "   Source : $LOCAL"
  ...
  case "$mode" in
    dry)
      for dir in "${dirs[@]}"; do
        echo "===== $dir ====="
        gdrive-copy-dir "$dir" dry || return 1
        echo
      done
      ;;
    run)
      for dir in "${dirs[@]}"; do
        echo "===== $dir ====="
        gdrive-copy-dir "$dir" run || return 1
        echo
      done
      ;;
    ask)
      ...
      read "ans?🚀 Execute backup for all dirs? (y/N): "
      [[ "$ans" != "y" ]] && echo "❌ Cancelled" && return 0
...
gdrive-sync-dir() {
  ...
  [[ -z "$dir" ]] && echo "❌ Usage: gdrive-sync-dir <dir> [ask|run|dry]" && return 1
  [[ ! -d "$LOCAL/$dir" ]] && echo "❌ Directory not found: $LOCAL/$dir" && return 1

  opts=($(rclone-common-opts))

  echo "⚠️ ONE-TIME SYNC (destructive)"
  echo "   Dir    : $dir"
  ...
  case "$mode" in
    ...
    ask)
      ...
      echo
      read "ans?🚨 REALLY sync '$dir'? type 'yes': "
...
gdrive-sync-once() {
  ...
  echo "⚠️ ONE-TIME SYNC (destructive)"
  echo "   This will DELETE files on remote!"
  echo "   Source : $LOCAL"
  echo "   Target : $REMOTE_DIR"
  echo "   Mode   : $mode"
  echo
  ...
  case "$mode" in
    dry)
      for dir in "${dirs[@]}"; do
        echo "===== $dir ====="
        gdrive-sync-dir "$dir" dry || return 1
        echo
      done
      ;;
    run)
      for dir in "${dirs[@]}"; do
        echo "===== $dir ====="
        gdrive-sync-dir "$dir" run || return 1
        echo
      done
      ;;
    ask)
      ...
      read "ans?🚨 REALLY sync ALL dirs? type 'yes': "
      [[ "$ans" != "yes" ]] && echo "❌ Cancelled" && return 0

      echo "🚀 EXECUTE SYNC (all dirs)"
      for dir in "${dirs[@]}"; do
        echo "===== $dir ====="
        gdrive-sync-dir "$dir" run || return 1
        echo
