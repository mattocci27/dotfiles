...
autossh-ali-check() {
  ...
  if ! ssh -G "$AUTOSSH_ALI_HOST" >/dev/null 2>&1; then
    echo "❌ SSH host '$AUTOSSH_ALI_HOST' is not resolvable via ssh config."
    echo "   Check ~/.ssh/config or AUTOSSH_ALI_HOST."
    return 1
  fi
}

# ------------------------------
# Main commands (Alibaba Cloud)
# ------------------------------
autossh-ali() {
  local remote_port="${1:-$AUTOSSH_ALI_REMOTE_PORT}"

  autossh-ali-check || return 1

  echo "🚇 Starting reverse SSH tunnel to Alibaba"
  echo "   Host        : $AUTOSSH_ALI_HOST"
  echo "   Remote port : $remote_port"
  echo "   Forward to  : localhost:22"
...
autossh-ali-bg() {
  ...
  echo "🚇 Starting reverse SSH tunnel to Alibaba in background"
  echo "   Host        : $AUTOSSH_ALI_HOST"
  echo "   Remote port : $remote_port"
  echo "   Forward to  : localhost:22"
  ...
  autossh -M 0 -N \
    ...
    -R "${remote_port}:localhost:22" \
    "$AUTOSSH_ALI_HOST" \
    >>"$AUTOSSH_ALI_LOG_OUT" 2>>"$AUTOSSH_ALI_LOG_ERR" &

  disown
  echo "✅ autossh (Alibaba) started in background"
}

autossh-ali-stop() {
  local remote_port="${1:-$AUTOSSH_ALI_REMOTE_PORT}"

  pkill -f "autossh.*-R ${remote_port}:localhost:22.*${AUTOSSH_ALI_HOST}" \
    && echo "🛑 autossh (Alibaba) stopped (port ${remote_port})" \
    || echo "❌ No matching autossh process found for Alibaba"
}

autossh-ali-status() {
  local remote_port="${1:-$AUTOSSH_ALI_REMOTE_PORT}"

  echo "🔍 autossh (Alibaba) process status (port ${remote_port})"
  pgrep -af "autossh.*-R ${remote_port}:localhost:22.*${AUTOSSH_ALI_HOST}" \
    || echo "❌ Not running"
}

autossh-ali-log() {
  echo "📜 autossh (Alibaba) logs"
  echo "   stdout: $AUTOSSH_ALI_LOG_OUT"
  echo "   stderr: $AUTOSSH_ALI_LOG_ERR"
  echo "----------------------------------------"
