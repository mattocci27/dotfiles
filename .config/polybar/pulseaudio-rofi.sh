...
outputs() {
    OUTPUT=$(pactl list short sinks | cut  -f 2 | rofi -theme sidetab -dmenu -p "Output" -mesg "Select prefered output source" )
...
inputs() {
    INPUT=$(pactl list short sources | cut  -f 2 | grep input | rofi -theme sidetab -dmenu -p "Output" -mesg "Select prefered input source" )
